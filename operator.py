...
class ExecuteBasix(bpy.types.Operator):
    bl_idname = "bim.execute_basix"
    bl_label = "Connect to BASIX"
    bl_options = {"REGISTER", "UNDO"}
 
    def execute(self, context):
        import bonsai.bim.extract
        bonsai.bim.extract.extract_basix(tool.Ifc.get())
        return {"FINISHED"}